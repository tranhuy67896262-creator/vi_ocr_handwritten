...
if [[ $# -gt 0 ]]; then
    ...
    echo "Dung: bash pipeline_train_20k.sh [3b|7b] [HF_TOKEN]"
    echo "Mac dinh gop data assets/ + repo. Tat: MERGE=0 bash pipeline_train_20k.sh [3b|7b] [HF_TOKEN]"
    exit 1
fi
if [[ ${#HF_TOKEN_ARG[@]} -eq 0 && -z "${HF_TOKEN:-}" ]] && ! grep -q '^HF_TOKEN' .env.dev 2>/dev/null; then
    echo "[ERR] Thieu HF_TOKEN cho gated dataset."
    echo "Dung: bash pipeline_train_20k.sh [3b|7b] hf_xxxxx"
