...
- `scripts/run_label_editor.bat` / `.sh` (hoặc `python -m scripts.labeling serve --port 9000`) — editor Bootstrap 5 ở `http://127.0.0.1:9000`. Nguồn `local` = `assets/` + `assets/labels.csv` (tự tạo 1 dòng rỗng/ảnh, tự backup `.bak`); nguồn `huggingface` load lazy `5CD-AI/Viet-Handwriting-OCR-v2`, sửa text lưu vào `assets/hf_overrides.csv` (không đụng dataset gốc). `--no-hf` để chỉ gắn nhãn ảnh cá nhân.
- `python -m scripts.labeling merge` gộp HF (đã áp override) + ảnh cá nhân → `data/combined/` (`DatasetDict`). `run_train.sh --merge` tự gộp rồi train `--dataset data/combined` (ngầm bật `--train`); vẫn cần GPU mạnh như train thường.
