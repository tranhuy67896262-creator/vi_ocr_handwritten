...
- UI (`scripts/ui.py`): dropdown Base model riêng mỗi tab, đổi model tự trỏ adapter về default (`sync_adapter`); nút nặng gọi `_free_gpu()` trước subprocess; Export 2 nút (thư mục + GGUF full-chain, GGUF yield 3-tuple, chỉ Linux, skip merge nếu fresh; `export_gguf.sh` convert 2 lần text + `--mmproj` vision rồi tự sinh `Modelfile` FROM+ADAPTER cho Ollama; `push_gguf_ui` đẩy cả model + mmproj cùng thư mục; nút import vào Ollama đang chạy (`import_ollama_ui` — check CLI + server + Modelfile, chạy `ollama create` với cwd là thư mục gguf); nút push adapter/GGUF lên Hub (cần HF_TOKEN Write + repo `owner/repo`); 403 = sai owner hoặc token Read. Tab Settings: lưu token (hiện tên tài khoản), kiểm tra cache model (`check_model_ui`).
