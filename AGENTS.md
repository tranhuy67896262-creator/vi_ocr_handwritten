...
- `run_train.sh` / `run_train.bat`: auto-cài uv + deps + torch-CUDA rồi gọi `train_qlora.py`. Token HF làm arg đầu (tự ghi `.env.dev`), mọi flag còn lại truyền thẳng qua. Ưu tiên `.venv` nếu có. Cả 2 set `UV_LINK_MODE=copy` và cache HF trong project (`.hf_cache`, tôn trọng `HF_HOME` có sẵn). **Mặc định (không cờ)**: chế độ nhẹ, chỉ cài `gradio`+`dotenv`+`pymupdf`+`python-docx` rồi mở UI ngay (nút Fine-tune/OCR báo lỗi nếu thiếu deps → chạy lại với `--train`). **Cờ riêng**: `--train` (train thật), `--ui` (giữ tương thích), `--eval[=N]` (eval sau train, mặc định 100). UI offline: `GRADIO_SHARE=0`.
